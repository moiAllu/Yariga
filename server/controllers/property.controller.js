...
const getPropertyDetail = async (req, res) => {
  const { id } = req.params;
  try {
    const propertyExits = await Property.findById({ _id: id }).populate(
      "creator"
    );
    if (!propertyExits) res.status(404).json({ message: "Property not found" });
...
const updateProperty = async (req, res) => {
  try {
    ...
    const { title, description, propertyType, location, price, photo, email } =
      req.body;
    const propertyToUpdate = await Property.findById({ _id: id }).populate(
      "creator"
    );
    if (!propertyToUpdate)
      res.status(404).json({ message: "Property not found" });
    ...
    propertyToUpdate.photo = photoUrl.url || propertyToUpdate.photo;
